...
@section('contents')
    ...
    <table class="table table-striped">
        <tbody>
        @foreach($seances->groupBy('semaine') as $semaine => $plannings)
            <tr>
                <td colspan="3" class="font-weight-bold">Semaine {{ $semaine }}</td>
            </tr>
            @foreach($plannings->groupBy('cours.intitule') as $cours => $plannings_cours)
                <tr>
                    <td colspan="3" class="font-weight-bold">{{ $cours }}</td>
                </tr>
                @foreach($plannings_cours as $planning)
                    <tr>
                        <td></td>
